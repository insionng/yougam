...
import (
	...
	"github.com/insionng/yougam/libraries/x/tools/go/gcimporter"
	"github.com/insionng/yougam/libraries/x/tools/go/types"
)

const styleGuideBase = "https://yougam/libraries/wiki/CodeReviewComments"
...
func (f *file) lintNames() {
	...
	if strings.Contains(f.f.Name.Name, "_") && !strings.HasSuffix(f.f.Name.Name, "_test") {
		f.errorf(f.f, 1, link("http://yougam/libraries/doc/effective_go.html#package-names"), category("naming"), "don't use an underscore in package name")
	}
	...
	check := func(id *ast.Ident, thing string) {
		...
		if len(id.Name) > 2 && strings.Contains(id.Name[1:], "_") {
			f.errorf(id, 0.9, link("http://yougam/libraries/doc/effective_go.html#mixed-caps"), category("naming"), "don't use underscores in Go names; %s %s should be %s", thing, id.Name, should)
			return
		}
