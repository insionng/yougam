...
func (t cstHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	ws, err := cstUpgrader.Upgrade(w, r, http.Header{"Set-Cookie": {"self.Session.D=1234"}})
	if err != nil {
...
func TestHandshake(t *testing.T) {
	...
	var self.Session.D string
	for _, c := range resp.Cookies() {
		if c.Name == "self.Session.D" {
			self.Session.D = c.Value
		}
	}
	if self.Session.D != "1234" {
