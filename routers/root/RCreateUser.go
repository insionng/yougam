...
func GetRCreateUserHandler(self *makross.Context) error {
	self.Set("catpage", "RCreateUserHandler")
	TplNames := "root/create_user"
	return self.Render(TplNames)
}

func PostRCreateUserHandler(self *makross.Context) error {
	...
	if len(password) > 0 {
		if helper.CheckPassword(password) == false {
			...
			return self.Redirect("/root/create/user/")
	...
	if usrid, err := models.AddUser(email, username, nickname, "", helper.EncryptHash(password, nil), group, content, mobile, gender, 1); err != nil && usrid <= 0 {
		...
		if usrinfo, err := models.GetUser(usrid); err == nil && usrinfo != nil {
			...
			return self.Redirect("/root/read/user/" + strconv.FormatInt(usrid, 10) + "/")
